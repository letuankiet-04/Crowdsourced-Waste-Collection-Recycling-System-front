...
export default function UserProfile({ user: propUser, className }) {
  ...
  const role = useMemo(() => (user?.role || user?.roleCode || "User").toUpperCase(), [user]);
  const enterpriseId = useMemo(() => user?.enterpriseId ?? user?.enterprise_id ?? null, [user]);
  const enterpriseName = useMemo(() => user?.enterpriseName ?? user?.enterprise_name ?? null, [user]);
  ...
  const createdAt = useMemo(() => formatDate(user?.createdAt), [user]);
  const lastLogin = useMemo(() => formatDateTime(user?.lastLogin), [user]);
