...
export default function CollectorReportDetail() {
  ...
  const initialWeightsByType = useMemo(() => {
  ...
  const initialCollectedAddress = useMemo(() => {
    if (typeof report?.collectedAddress === "string" && report.collectedAddress.trim()) return report.collectedAddress;
    if (typeof report?.address === "string") return report.address;
    return "";
  }, [report]);

  const initialCollectedCoords = useMemo(() => {
    return report?.collectedCoords ?? report?.coords ?? null;
  }, [report]);
    ...
    <CollectorLayout>
      <div className="space-y-8">
        ...
        <CollectReportDialog
          ...
          statusLabel={status}
          statusVariant={reportStatusToPillVariant(status)}
          wasteTypes={wasteTypes}
          initialAddress={initialCollectedAddress}
          initialCoords={initialCollectedCoords}
          initialWeightsByType={initialWeightsByType}
